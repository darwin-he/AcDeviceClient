...
public class StepperMotorImpl implements StepperMotor {
     ...
     */
    @Override
    public void openDoor(OnTurned onBack) {
        if (!isReadyToManipulation()) {
            onBack.onTurned(false);
            return;
        }
        if (isOpended) {
            onBack.onTurned(true);
            return;
        }
        isTurning = true;
     ...
     */
    @Override
    public void closeDoor(OnTurned onBack) {
        if (!isReadyToManipulation()) {
            onBack.onTurned(false);
            return;
        }
    ...
    public StepperState getState() {
        ...
        stepperState.setOpened(isOpended);
        stepperState.setTurning(isTurning);
        return stepperState;
    }
     ...
     */
    private void uniformTurnCW(int stepCount, long frequency, OnTurned onBack) {
        setAllGpioPinToLow();
        new Thread(() -> {
     ...
     * @param onBack    转动完成回调函数
     */
    private void uniformTurnCCW(int stepCount, long frequency, OnTurned onBack) {
        setAllGpioPinToLow();
        new Thread(() -> {
            for (int i = stepCount; i > 0; i--) {
